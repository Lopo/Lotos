...
GOT_LINE:
    ...
    /* If a dot on its own then execute last inpstr unless its a misc
       op or the user is on a remote site */
			if (!user->misc_op) {
				if (!strcmp(inpstr,".") && user->inpstr_old[0] && user->level>GOD) {
...
int has_gcom(UR_OBJECT user, int cmd_id)
{
	...
	for (i=0;i<MAX_GCOMS;i++)
		if (user->gcoms[i]==cmd_id) return 1;
	return 0;
}
...
int has_xcom(UR_OBJECT user, int cmd_id)
{
	...
	for (i=0;i<MAX_XCOMS;i++)
		if (user->xcoms[i]==cmd_id) return 1;
	return 0;
}
...
if (strcmp(USERVER, ver=remove_first(line))) {
	fclose(fp);
	i=load_user_olddetails(user, ver);
	return i;
	}
...
int load_user_olddetails(UR_OBJECT user, char *ver)
{
	if (!strcmp(ver, "0.10")) return(load_user_olddetails010(user));
	if (!strcmp(ver, "0.11")) return(load_user_olddetails011(user));
	return 0;
}

/*** Load the old users details v0.10 ***/
int load_user_olddetails010(UR_OBJECT user)
{
	FILE *fp;
	...
	set_crash();
	write_syslog(SYSLOG, 1, "Nahravam staru verziu userfajlu pre %s, v0.1 > 1.0.0-1.1.1\n", user->name);
...
/*** Load the old users details v0.11 ***/
int load_user_olddetails011(UR_OBJECT user)
{
	FILE *fp;
	char user_words[16][40]; /* must be at least 1 longer than max words in the _options */
	char line[ARR_SIZE],fname[500],*str;
	...
	set_crash();
	write_syslog(SYSLOG, 1, "Nahravam staru verziu userfajlu pre %s, v0.11 > 1.2.0\n", user->name);
	sprintf(fname,"%s/%s.D", USERFILES, user->name);
	if (!(fp=fopen(fname,"r"))) return 0;
...
{
	struct user_dir_struct *entry=NULL;
	int i,lines=0,retval=1;
...
void login(UR_OBJECT user, char *inpstr)
{
  ...
  case LOGIN_PASSWD:
	...
	if (amsys->motd2_cnt) {
	  ...
	  if (!show_file(user,motdname)) {
	  	write_syslog(ERRLOG, 1, "nemozem zobrazit: %s\n", motdname);
	  	}
	  }
...
{
	int i,d=tmday,m=tmonth+1,y=tyear,cnt;
