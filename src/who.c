/*****************************************************************************
                  Funkcie OS Star v1.1.0 pre zoznamy typu who
...
void who_nuts333(UR_OBJECT user)
{
	...
	for (u=user_first; u!=NULL; u=u->next) {
		...
		sprintf(text, "%-*s : %-4s : %-12s : %d mins.", 40+cnt*3, line, user_level[u->level].name, rname, mins);
...
/*               if (!cu->level && cu->muzzled & F_SCUM) strcpy(levelname,"Scum");
          ...
          if (u->unarrest) strcpy(levelname,"Jailed");
